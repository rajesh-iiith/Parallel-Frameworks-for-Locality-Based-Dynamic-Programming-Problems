...
__global__ void update_array_gpu_tiled(int mode, int i, cellType *d_array, int *d_v, int *d_w, int dependencyWidthLeft)
{
   ...
   const int columnWidth = TILE_COLS + 10;
   ...
   if (threadIdx.x >= dependencyWidthLeft)
   	{
		a = jointArray[columnWidth + threadIdx.x];
		b = d_v[r] + jointArray[columnWidth + threadIdx.x - d_w[r]];
		(( (d_w[r] > myCol) || (a >= b)) ? jointArray[threadIdx.x] = a : jointArray[threadIdx.x] = b );
   ...
   for (int iter = 2; iter <= TILE_ROWS; ++iter)
   {
	 	...
	 	__syncthreads();
	   	...
	   	if (threadIdx.x >= dependencyWidthLeft)
	   	{	
	   		a = jointArray[oe_eo + threadIdx.x];
			b = d_v[myRow] + jointArray[oe_eo + threadIdx.x - d_w[myRow]];
			(( (d_w[myRow] > myCol) || (a >= b)) ? jointArray[oo_ee + threadIdx.x] = a : jointArray[oo_ee + threadIdx.x] = b );
